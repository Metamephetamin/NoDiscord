...
public class StreamedChatFileUploadReaderTests : IDisposable
{
    ...
    [Fact]
    public async Task UploadAsync_UsesParsedFormFileWhenFormWasReadBeforeStreamingReader()
    {
        var fileBytes = Encoding.UTF8.GetBytes("hello from already parsed form upload");
        var request = BuildMultipartRequest("upload.txt", "text/plain", fileBytes);
        await request.ReadFormAsync();
        request.Body = new ThrowingReadStream();
    ...
    [Fact]
    public async Task UploadAsync_RejectsInstallerFileBeforeStorageWrite()
    {
    ...
    private sealed class ThrowingReadStream : Stream
    {
        ...
        {
            throw new IOException("Request body should not be read after form has already been parsed.");
        }
