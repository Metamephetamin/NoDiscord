...
public static class DatabaseSchemaInitializer
{
    public static async Task InitializeAsync(IServiceProvider services)
    {
        ...
        await dbContext.Database.ExecuteSqlRawAsync(
            ...
            CREATE UNIQUE INDEX IF NOT EXISTS ix_user_e2ee_keys_user_id ON user_e2ee_keys (user_id);
            CREATE INDEX IF NOT EXISTS ix_user_e2ee_keys_fingerprint ON user_e2ee_keys (fingerprint);

            CREATE TABLE IF NOT EXISTS channel_e2ee_daily_keys (
                id integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                scope character varying(16) NOT NULL,
                channel_id character varying(160) NOT NULL,
                key_date character varying(10) NOT NULL,
                recipient_user_id integer NOT NULL REFERENCES users(id) ON DELETE CASCADE,
                creator_user_id integer NOT NULL REFERENCES users(id) ON DELETE CASCADE,
                creator_fingerprint character varying(128) NOT NULL,
                creator_public_key_jwk text NOT NULL,
                wrap_iv character varying(256) NOT NULL,
                wrapped_key text NOT NULL,
                created_at timestamptz NOT NULL,
                updated_at timestamptz NOT NULL
            );
            CREATE UNIQUE INDEX IF NOT EXISTS ix_channel_e2ee_daily_keys_scope_channel_date_recipient
                ON channel_e2ee_daily_keys (scope, channel_id, key_date, recipient_user_id);
            CREATE INDEX IF NOT EXISTS ix_channel_e2ee_daily_keys_scope_channel_date
                ON channel_e2ee_daily_keys (scope, channel_id, key_date);
