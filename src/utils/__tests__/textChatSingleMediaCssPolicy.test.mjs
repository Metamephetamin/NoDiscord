...
test("single video messages keep a concrete responsive width in every chat", () => {
  assert.match(
    css,
    /\.msg-content--single-video-only\s+\.message-media-overlay-anchor[\s\S]*?\.msg-content--single-video-only\s+\.message-attachment-single[\s\S]*?\.msg-content--single-video-only\s+\.message-attachment-single\s+\.message-media[\s\S]*?\.msg-content--single-video-only\s*>\s*\.message-media--video\s*\{[\s\S]*?width:\s*min\(620px,\s*calc\(100vw\s*-\s*160px\)\)\s*!important;/,
    "single video shells need a viewport-based width instead of percentage width inside fit-content containers",
  );
  assert.match(
    css,
    /\.message-item--dm\s+\.msg-content--single-video-only\s+\.message-media-overlay-anchor[\s\S]*?\.message-item--dm-own\s+\.msg-content--single-video-only\s*>\s*\.message-media--video\s*\{[\s\S]*?width:\s*min\(620px,\s*calc\(100vw\s*-\s*128px\)\)\s*!important;/,
    "direct chat single videos need a narrower viewport-based width for both incoming and own messages",
  );
});
