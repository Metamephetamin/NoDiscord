...
export async function sendOutboxMessageViaHttp({ targetChannelId, avatar, item, preparedTextPayload }) {
  ...
  const response = await authFetch(`${API_BASE_URL}/chats/${encodeURIComponent(targetChannelId)}/messages/outbox`, {
    ...
    body: JSON.stringify({
      ...
      photoUrl: avatar || "",
      clientMessageId: item?.clientMessageId || item?.ClientMessageId || item?.clientTempId || item?.ClientTempId || "",
      clientTempId: item?.clientTempId || item?.ClientTempId || item?.clientMessageId || item?.ClientMessageId || "",
